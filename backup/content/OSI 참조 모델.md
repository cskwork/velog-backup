...
### 물리 계층
- 컴퓨터를 서로 연결하는 물리 장비
- 무엇을 통해 어떻게 신호를 전송하는지 정의

- 전송에 필요한 두 장치 간의 접속과 절단 등 기계적, 전기적, 기능적, 절차적 특성에 대한 규칙
- 물리적 전송 매체와 전송 신호 방식 정의.

https://www.notion.so/23d54dc247114061a543aa1a9aa6f5ce

### 데이터 링크 계층
- 전송 신호를 해석하는 방법을 정의해서 연결 신뢰성 보장.
...
### 네트워크 계층
- 네트워크끼리의 통신을 다룬다. 
- 라우터로 연결된 네트워크 집합을 인터네트워크(Internetwork)라 하며 이 중 가장 유명한 것이 인터넷(Internet)이다. 
- 데이터링크 계층에서 하나의 링크를 통해 데이터를 가져왔다면, 네트워크 계층에서는 네트워크 집합으로부터 데이터를 가져온다.
...
### 전송 계층
-전송하는 데이터의 오류를 점검하고, 데이터의 목적지가 어떤 어플리케이션인지를 식별한다.
...
### 응용 계층
- 사용자에게 가장 밀접한/익숙한 계층
- 대표적으로 SMTP(이메일 프로토콜), FTP(파일 전송 프로토콜), HTTP(하이퍼텍스트 전송 프로토콜).
